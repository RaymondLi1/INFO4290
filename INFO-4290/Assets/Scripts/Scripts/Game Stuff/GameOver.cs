...
public class GameOver : MonoBehaviour
{
    ...
    public void gameOver()
    {
        ...
        FindObjectOfType<PlayerController>().currentState = PlayerState.idle;
        FindObjectOfType<PlayerController>().animator.SetBool("moveing", false);
        FindObjectOfType<AudioManager>().stopAllThemes();
        FindObjectOfType<AudioManager>().Play("Game Over Theme");
        if (FindObjectOfType<AudioManager>().findAudioSource("running").isPlaying)
        {
            FindObjectOfType<AudioManager>().findAudioSource("running").volume = 0f;
        }
