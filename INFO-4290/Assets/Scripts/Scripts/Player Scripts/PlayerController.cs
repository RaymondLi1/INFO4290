...
public class PlayerController : MonoBehaviour
{
    ...
    {
        if (UIActive.RunTimeValue || currentState == PlayerState.interact)
            return;
        myRigidbody.velocity = Vector2.zero;
        if (context.performed && currentState != PlayerState.attack && currentState != PlayerState.stagger && currentState != PlayerState.dash && currentState != PlayerState.interact)
        {
    ...
    public void Parry(InputAction.CallbackContext context)
    {
        if (UIActive.RunTimeValue || currentState == PlayerState.interact)
            return;
        if (context.performed && currentState != PlayerState.attack && currentState != PlayerState.stagger && daggerUsable() && currentState != PlayerState.dash && currentState != PlayerState.interact)
        {
